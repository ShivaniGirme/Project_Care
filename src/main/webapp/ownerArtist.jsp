...
<title>OwnerArtist</title><meta http-equiv="X-UA-Compatible" content="IE=edge">
...
table {
    ...
    padding: 15px; /* Padding around the cells */
    justify-content: center;
}
td, th {
...
<body class="homepage">
  ...
  <nav class="navbar navbar-expand-lg text-uppercase fs-6 py-3 px-0 px-md-3 border-bottom align-items-center">
	...
	<%
		...
		<div class="container">
			<%
			ArtistCRUD artist1 = new ArtistCRUD();
			List<Artist> alist = artist1.getAllArtistByUserId(user.getUserId());
			%>
			...
			<thead>
				...
				%>
				<tr>
					<td scope="row"><%=artist.getArtistId()%></td>
					<td><%=artist.getName()%></td>
					<td><%=artist.getEmail()%></td>
					<td><%=artist.getPhone()%></td>
					<td><%=artist.getFees()%></td>
					<td><%=artist.getExperience()%></td>
					<td><%=artist.getEducation()%></td>
					<td><%=artist.getSpecialization()%></td>
					<td><a href="updateartist?id=<%=artist.getArtistId()%>"><button>Update</button></a></td>
					<td><a href="deleteartist?id=<%=artist.getArtistId()%>"><button>Delete</button></a></td>
